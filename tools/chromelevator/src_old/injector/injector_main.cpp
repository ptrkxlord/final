...
void ProcessBrowser(const BrowserInfo& browser, bool verbose, bool fingerprint, bool killFirst,
    ...
    try {
        ...
        // V6.18: Only terminate if we created it (don't kill attached telegram etc.)
        if (existingPid == 0) {
            procMgr.Terminate();
        }
...
extern "C" __declspec(dllexport) int __stdcall RunChromeEngine(int argc, wchar_t* argv[]) {
    bool verbose = false;
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        if (arg == L"--verbose" || arg == L"-v") verbose = true;
            ...
            // Priority to "all" command
            if (arg == L"all") targetType = L"all";
            else if (targetType.empty() || targetType == L"all") targetType = arg;
        }
    }
